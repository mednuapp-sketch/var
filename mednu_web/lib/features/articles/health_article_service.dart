...
/// Same `health_articles` collection the mednu patient app already streams
/// (see mednu/lib/features/education/services/health_article_service.dart)
/// — reused here rather than duplicating content into this app.
class HealthArticle {
  final String id;
  final String title;
...
class HealthArticleService {
  ...
  static Stream<HealthArticle?> streamOne(String id) => _db
      .collection('health_articles')
      .doc(id)
      .snapshots()
      .map((d) => d.exists ? HealthArticle.fromFirestore(d) : null);
