...
class MapsLauncher {
  ...
  /// Opens Google Maps navigation to a free-text destination (e.g. a
  /// clinic's street address) when no lat/lng is available.
  static Future<void> navigateToAddress(String address) async {
    final uri = Uri.parse(
      'https://www.google.com/maps/dir/?api=1'
      '&destination=${Uri.encodeComponent(address)}&travelmode=driving',
    );
    if (await canLaunchUrl(uri)) {
      await launchUrl(uri, mode: LaunchMode.externalApplication);
    }
