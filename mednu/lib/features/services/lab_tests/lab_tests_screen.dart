...
class _LabTestsScreenState extends ConsumerState<LabTestsScreen> {
  static const _themeColor = Color(0xFF3949AB);
  ...
  static const _icons = [
    Icons.bloodtype_rounded, Icons.water_drop_rounded, Icons.favorite_rounded,
    Icons.monitor_heart_rounded, Icons.wb_sunny_rounded, Icons.science_rounded,
          ...
          SliverToBoxAdapter(
            child: StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance
                  .collection('services')
                  .where('type', isEqualTo: 'lab_tests')
                  .where('isEnabled', isEqualTo: true)
                  .snapshots(),
              builder: (ctx, snap) {
                return Column(children: [
                  // ── My Lab Test Bookings (realtime) ───────────────────────
                  const _MyLabTestBookings(),
                  Padding(
                  ...
                  // ── Popular tests ─────────────────────────────────────────
                  _PopularLabTestsRow(onBook: _book),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (filtered.isEmpty)
        Padding(
          ...
          child: Column(children: [
            Icon(_query.isNotEmpty ? Icons.search_off_rounded : Icons.bloodtype_outlined, size: 48, color: Colors.grey[300]),
...
// ── Popular Lab Tests Row ───────────────────────────────────────────────────

class _PopularLabTestsRow extends StatelessWidget {
  final void Function(Map<String, dynamic>) onBook;
  const _PopularLabTestsRow({required this.onBook});
...
class _MyLabTestBookings extends StatelessWidget {
  const _MyLabTestBookings();
...
class _LabTestBookingCard extends StatelessWidget {
  ...
  const _LabTestBookingCard({required this.booking, required this.onTap});
