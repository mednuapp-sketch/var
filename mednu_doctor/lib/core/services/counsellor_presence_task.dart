...
// CounsellorPresenceService before starting the foreground service so the
// background isolate (which cannot access FirebaseAuth) knows which
// counsellor to send heartbeats for.
const kCounsellorPresenceUidKey = 'mednu_doctor_counsellor_presence_uid';
...
void counsellorPresenceTaskCallback() {
  FlutterForegroundTask.setTaskHandler(CounsellorPresenceTaskHandler());
}

// ── Task handler — runs in a dedicated background isolate ─────────────────────
// Same shape as DoctorPresenceTaskHandler/AmbulancePresenceTaskHandler,
// targeting `counsellor_profiles` instead.
class CounsellorPresenceTaskHandler extends TaskHandler {
  ...
  Future<void> onDestroy(DateTime timestamp) async {
    ...
    try {
      await _db?.collection('counsellor_profiles').doc(uid).update({
        'isOnline': false,
  ...
  Future<String?> _getUid() async {
    try {
      ...
      return prefs.getString(kCounsellorPresenceUidKey);
    } catch (_) {
      return null;
    }
  }
